...
export const Profile = () => {
  ...
  const emergencyContacts = Array.isArray(user?.emergencyContacts) ? user.emergencyContacts : [];
  ...
  const avatarSize = isMobile ? 84 : 96;

  return (
    <div style={{ maxWidth: 960, margin: '0 auto', width: '100%', display: 'flex', flexDirection: 'column', gap: 20, paddingBottom: 20 }}>
      {saveMsg && (
        <div
          style={{
        ...
        >
          <span className="material-symbols-outlined" style={{ fontSize: 18 }}>
            {saveMsg.includes('Failed') ? 'error' : 'check_circle'}
          </span>
          {saveMsg}
        </div>
      )}
      ...
      <div style={sectionCardStyle}>
        ...
        <div style={{ padding: isMobile ? 20 : 28, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, textAlign: 'center' }}>
          <div style={{ position: 'relative' }}>
            ...
            >
              {uploading ? (
                <span className="material-symbols-outlined" style={{ color: '#fff', fontSize: 14, animation: 'spin 1s linear infinite' }}>
                  progress_activity
                </span>
              ) : (
            ...
            <>
              <div style={{ width: '100%', maxWidth: 640 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
                  <h2 style={{ fontSize: isMobile ? 24 : 28, fontWeight: 800, margin: 0, lineHeight: 1.15, wordBreak: 'break-word' }}>
                    {user?.name || 'User'}
                  </h2>
                  <div
                    style={{
                      ...
                      alignItems: 'center',
                      gap: 4,
                      background: `${league.current.color}15`,
                      color: league.current.color,
                      padding: '4px 10px',
                      borderRadius: 12,
                      border: `1px solid ${league.current.color}40`,
                      ...(league.current.name === 'Shikhar' ? { boxShadow: `0 0 12px ${league.current.color}50` } : {}),
                    }}
                  >
                ...
                {form.bio && <p style={{ fontSize: 15, color: '#475569', margin: '6px 0 0', lineHeight: 1.6 }}>{form.bio}</p>}
                {league.next && (
                ...
                {form.location_text && (
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, marginTop: 6, fontSize: 13, color: '#737685', flexWrap: 'wrap' }}>
                    <span className="material-symbols-outlined" style={{ fontSize: 15 }}>
                      location_on
                    </span>
                    <span style={{ wordBreak: 'break-word' }}>{form.location_text}</span>
                  </div>
                )}
              </div>
            ...
            <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 14, textAlign: 'left' }}>
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                <button
                  ...
                  style={{ flex: isMobile ? '1 1 100%' : 1, padding: '12px', borderRadius: 10, border: '1.5px solid #e1e2e4', background: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}
                >
                  Cancel
                </button>
                <button
                  onClick={handleSave}
                  disabled={saving}
                  style={{ flex: isMobile ? '1 1 100%' : 2, padding: '12px', borderRadius: 10, border: 'none', background: '#0052cc', color: '#fff', fontSize: 14, fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer', fontFamily: 'Inter, sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, opacity: saving ? 0.7 : 1 }}
                >
                  {saving && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, minmax(0, 1fr))', gap: 12 }}>
        {[
          ...
          <div key={stat.label} style={{ ...sectionCardStyle, padding: '18px 16px', textAlign: 'center' }}>
            <span className="material-symbols-outlined" style={{ fontSize: 24, color: stat.accent, fontVariationSettings: "'FILL' 1" }}>
              {stat.icon}
            </span>
            <div style={{ fontSize: 28, fontWeight: 800, color: '#191c1e', marginTop: 8 }}>{stat.value}</div>
            <div style={{ fontSize: 11, color: '#737685', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', marginTop: 6 }}>
              {stat.label}
            </div>
          </div>
        ))}
      ...
      <div style={sectionCardStyle}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #f8f9fb', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <div>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'minmax(0,1fr) auto', gap: 10 }}>
              <input
                ...
                style={{ width: '100%', padding: '12px 14px', borderRadius: 10, border: '1.5px solid #e1e2e4', fontSize: 14, fontFamily: 'Inter, sans-serif' }}
              />
              <button onClick={handleSendCode} disabled={sendingCode} style={{ padding: '12px 16px', borderRadius: 10, border: 'none', background: '#0052cc', color: '#fff', fontWeight: 700, cursor: 'pointer', opacity: sendingCode ? 0.7 : 1, width: isMobile ? '100%' : 'auto' }}>
          ...
          {verificationId && (
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'minmax(0,1fr) auto', gap: 10 }}>
              <input
                ...
                style={{ width: '100%', padding: '12px 14px', borderRadius: 10, border: '1.5px solid #e1e2e4', fontSize: 14, letterSpacing: '0.3em', fontFamily: 'Inter, sans-serif' }}
              />
              <button onClick={handleVerifyCode} disabled={verifyingCode} style={{ padding: '12px 16px', borderRadius: 10, border: 'none', background: '#15803d', color: '#fff', fontWeight: 700, cursor: 'pointer', opacity: verifyingCode ? 0.7 : 1, width: isMobile ? '100%' : 'auto' }}>
                {verifyingCode ? 'Verifying...' : 'Confirm OTP'}
              </button>
            </div>
          )}

          {phoneMessage && <p style={{ margin: 0, fontSize: 13, color: phoneMessage.toLowerCase().includes('success') || phoneMessage.toLowerCase().includes('sent') ? '#166534' : '#b45309', lineHeight: 1.5 }}>{phoneMessage}</p>}
      ...
      <div style={sectionCardStyle}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {[
            ...
            { icon: 'fingerprint', label: 'User ID', value: user?.id ? `${user.id.slice(0, 12)}...` : '-' },
          ].map((item, index) => (
            <div key={item.label} style={{ display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', padding: '14px 20px', borderBottom: index < 4 ? '1px solid #f8f9fb' : 'none' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: '#f8f9fb', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <span className="material-symbols-outlined" style={{ fontSize: 18, color: '#737685' }}>
                    {item.icon}
                  </span>
                </div>
                <span style={{ fontWeight: 600, fontSize: 14, color: '#191c1e' }}>{item.label}</span>
              </div>
              <span style={{ fontSize: 14, color: '#737685', fontWeight: 500, wordBreak: 'break-word', width: isMobile ? '100%' : 'auto', textAlign: isMobile ? 'left' : 'right' }}>{item.value}</span>
            </div>
          ))}
        </div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'minmax(0,1.3fr) minmax(0,1fr)', gap: 20 }}>
        <div style={sectionCardStyle}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid #f8f9fb' }}>
          ...
          <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {sortedReports.slice(0, 4).map((report) => (
              <div key={report.id} style={{ padding: '12px 14px', borderRadius: 14, background: '#f8fafc', border: '1px solid #e2e8f0' }}>
                <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: '#0f172a', wordBreak: 'break-word' }}>{report.title}</p>
                <p style={{ margin: '4px 0 0', fontSize: 12, color: '#64748b', lineHeight: 1.5, wordBreak: 'break-word' }}>
            ...
            {badges.map((badge) => (
              <div key={badge.key} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', borderRadius: 14, padding: '12px 14px', background: badge.earned ? `${badge.accent}10` : '#f8fafc', border: `1px solid ${badge.earned ? `${badge.accent}40` : '#e2e8f0'}`, opacity: badge.earned ? 1 : 0.65 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: '#0f172a', wordBreak: 'break-word' }}>{badge.name}</p>
                  <p style={{ margin: '4px 0 0', fontSize: 12, color: '#64748b', lineHeight: 1.5 }}>{badge.description}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={sectionCardStyle}>
        ...
        <div style={{ padding: '16px 20px', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2, minmax(0, 1fr))', gap: 12 }}>
          <div style={{ borderRadius: 14, background: '#f8fafc', border: '1px solid #e2e8f0', padding: '14px' }}>
            ...
            <p style={{ margin: '8px 0 0', fontSize: 14, color: '#0f172a', lineHeight: 1.6, wordBreak: 'break-word' }}>{emergencyContacts.join(', ') || 'No contacts added yet'}</p>
          </div>
          <div style={{ borderRadius: 14, background: '#f8fafc', border: '1px solid #e2e8f0', padding: '14px' }}>
            <p style={{ margin: 0, fontSize: 11, color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Skills</p>
            <p style={{ margin: '8px 0 0', fontSize: 14, color: '#0f172a', lineHeight: 1.6, wordBreak: 'break-word' }}>{skills.join(', ') || 'No skills listed yet'}</p>
          </div>
        </div>
      </div>
      ...
      {showAvatarPreview && (
        <div onClick={() => setShowAvatarPreview(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.78)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 110, padding: isMobile ? 16 : 24 }}>
          <div onClick={(event) => event.stopPropagation()} style={{ background: '#fff', borderRadius: 24, padding: isMobile ? 20 : 28, maxWidth: 420, width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}>
            {renderAvatar(isMobile ? 180 : 220)}
            <div style={{ textAlign: 'center' }}>
              <h3 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: '#0f172a', wordBreak: 'break-word' }}>{user?.name || 'User'}</h3>
              <p style={{ margin: '6px 0 0', fontSize: 14, color: '#64748b' }}>Profile preview</p>
            </div>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center', width: '100%' }}>
              <button onClick={() => fileRef.current?.click()} style={{ padding: '12px 16px', borderRadius: 12, border: 'none', background: '#0052cc', color: '#fff', fontWeight: 700, cursor: 'pointer', width: isMobile ? '100%' : 'auto' }}>
                Change Photo
              </button>
              {user?.photoURL && (
                <button onClick={handleRemovePhoto} disabled={removingPhoto} style={{ padding: '12px 16px', borderRadius: 12, border: '1px solid #fecaca', background: '#fff5f5', color: '#b91c1c', fontWeight: 700, cursor: 'pointer', width: isMobile ? '100%' : 'auto' }}>
