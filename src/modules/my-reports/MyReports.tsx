...
export const MyReports = () => {
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', width: '100%', display: 'flex', flexDirection: 'column', gap: 24, paddingBottom: 80 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, margin: 0, letterSpacing: '-0.5px' }}>My Reports</h1>
          <p style={{ fontSize: 15, color: '#737685', margin: '4px 0 0' }}>Track live status, reopen resolved cases, and submit response feedback.</p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', background: '#fff', borderRadius: 12, border: '1px solid #e1e2e4', fontWeight: 600, fontSize: 13, color: '#191c1e' }}>
      ...
      {filtered.map((report) => {
        ...
        return (
          <article key={report.id} style={{ background: '#fff', borderRadius: 18, border: '1px solid #f1f5f9', boxShadow: '0 2px 12px rgba(0,0,0,0.05)', overflow: 'hidden', position: 'relative' }}>
            ...
            <div style={{ padding: '20px 20px 20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <h2 style={{ fontSize: 17, fontWeight: 700, margin: 0, color: '#191c1e', lineHeight: 1.3 }}>{report.title}</h2>
                  {report.location && (
                  ...
                  {report.description && <p style={{ fontSize: 13, color: '#434654', margin: '6px 0 0', lineHeight: 1.5 }}>{report.description}</p>}
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8, flexShrink: 0 }}>
                  ...
                  {canDelete ? (
                    <button
                      ...
                      style={{ padding: '4px 10px', borderRadius: 8, border: '1px solid #fecaca', background: '#fef2f2', color: '#b91c1c', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'Inter, sans-serif', display: 'flex', alignItems: 'center', gap: 4, opacity: deleting === report.id ? 0.5 : 1 }}
                    >
                    ...
                    <button
                      ...
                      style={{ padding: '4px 10px', borderRadius: 8, border: '1px solid #bfdbfe', background: '#eff6ff', color: '#1d4ed8', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'Inter, sans-serif', opacity: reopeningReportId === report.id ? 0.6 : 1 }}
                    >
              ...
              {Array.isArray(report.imageUrls) && report.imageUrls.length > 0 && (
                <div style={{ display: 'grid', gridTemplateColumns: report.imageUrls.length > 1 ? 'repeat(2, 1fr)' : '1fr', gap: 10 }}>
                  {report.imageUrls.slice(0, 4).map((imageUrl: string) => (
                    <img key={imageUrl} src={imageUrl} alt="Report evidence" style={{ width: '100%', height: 124, objectFit: 'cover', borderRadius: 14, border: '1px solid #e5e7eb' }} />
                  ))}
                </div>
              )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {timeline.map((event: any) => (
                    <div key={`${report.id}-${event.key}-${event.label}`} style={{ display: 'flex', gap: 10 }}>
                      ...
                      <div>
                        <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: '#0f172a' }}>{event.label}</p>
                ...
                <section style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 12 }}>
                  {report.assignedResponderName && (
                    <div style={{ background: '#fff7ed', borderRadius: 12, padding: '12px 14px', border: '1px solid #fed7aa' }}>
                      ...
                      <p style={{ margin: '6px 0 0', fontSize: 14, fontWeight: 700, color: '#7c2d12' }}>{report.assignedResponderName}</p>
                    </div>
                  )}
                  {report.etaText && (
                    <div style={{ background: '#eff6ff', borderRadius: 12, padding: '12px 14px', border: '1px solid #bfdbfe' }}>
                      ...
                      <p style={{ margin: '6px 0 0', fontSize: 14, fontWeight: 700, color: '#1e3a8a' }}>{report.etaText}</p>
                    </div>
                  )}
                  {report.progressNote && (
                    <div style={{ background: '#f0fdf4', borderRadius: 12, padding: '12px 14px', border: '1px solid #bbf7d0' }}>
                      ...
                      <p style={{ margin: '6px 0 0', fontSize: 14, fontWeight: 700, color: '#166534' }}>{report.progressNote}</p>
                    </div>
                  )}
                </section>
              )}
              ...
              {report.status === 'resolved' && (
                <section style={{ background: '#f8fafc', borderRadius: 14, padding: '14px 16px', border: '1px solid #e2e8f0' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                    ...
                    {report.feedbackSubmittedAt && (
                      <span style={{ fontSize: 12, fontWeight: 700, color: '#15803d' }}>Feedback saved</span>
                    )}
                  </div>
                  ...
                  <button
                    ...
                    style={{ marginTop: 12, padding: '12px 16px', borderRadius: 10, border: 'none', background: '#0052cc', color: '#fff', fontWeight: 700, cursor: draft.rating ? 'pointer' : 'not-allowed', opacity: savingFeedback === report.id || !draft.rating ? 0.7 : 1 }}
                  >
